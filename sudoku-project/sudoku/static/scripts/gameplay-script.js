...
${
	(index - Math.floor(index / 9) * 9) % 3 === 2 &&
	index - Math.floor(index / 9) * 9 !== 8
		? "rightborder"
		: ""
}
'>
...
const gridHTML = [...puzzle2].map((value, index) => createGrid(value, index))
	.join``;
...
var lastSelected = "";
const numButtons = document
	.getElementById("numButtonsContainer")
	.getElementsByClassName("numButton");
...
function enterNum(targetButton) {
	for (i = 0; i < 81; i++) {
		if (
			...
			cells[i].classList.contains("canEdit")
		) {
